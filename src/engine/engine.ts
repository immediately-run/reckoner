...
export class Engine {
   ...
   * per subject cell. A test's declared inputs resolve against the same published state a
   * cell's do (§6, R3-373) and split two ways:
   *
   * - **name-matched → substitution** — they override the subject's live inputs by local
   *   name, the subject formula **re-runs** over the merged set, and `expect`/`relation`
   *   assert against that substituted run (the holdout shape: swap the data input for its
   *   fixture, keep params/static live);
   * - **name-unmatched → auxiliary** — they are context for `expect`/`relation` only (an
   *   oracle fixture) and never feed the formula.
   *
   * Metamorphic invariance relations re-run the subject formula over a transformed input
   * via the injected `reevaluate` port (the transformed re-run rides the same merged base).
   */
  runTests(): Map<string, SuiteResult> {
    ...
    for (const [subject, tests] of this.testsBySubject) {
      ...
      const liveSubject = (): Value => this.scheduler.result(subject)?.value ?? null;
      const suite = runSuite(tests, (test) => {
        ...
        if (subjectDef === undefined) {
          return { subject: liveSubject(), inputs: liveInputs };
        }
        if (Object.keys(test.inputs).length === 0) {
          return { subject: liveSubject(), inputs: liveInputs, reevaluate };
        }
        ...
        for (const r of resolvers) {
          if (r.kind === 'external' && !this.scheduler.hasExternal(r.key)) {
            return { subject: null, inputs: {}, error: `test "${subject}" input "${r.name}" references absent external "${r.key}".` };
          }
          if (r.kind === 'windowed-feed' && !this.scheduler.hasExternal(`${FEED_BUFFER_PREFIX}${r.feed}`)) {
            return { subject: null, inputs: {}, error: `test "${subject}" windowed input "${r.name}" references feed "${r.feed}", which has no retained buffer.` };
          }
        }
        ...
        for (const [name, value] of Object.entries(declared)) {
          if (name in liveInputs) substituted[name] = value;
          else auxiliary[name] = value;
        }
        if (Object.keys(substituted).length === 0) {
          return { subject: liveSubject(), inputs: { ...liveInputs, ...auxiliary }, reevaluate };
        }
        const merged = { ...liveInputs, ...substituted };
        try {
          return { subject: subjectDef.formula(merged), inputs: { ...merged, ...auxiliary }, reevaluate };
        } catch (e) {
