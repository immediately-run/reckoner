...
the S5 spike doc). **298 vitest cases**; every merge is green on `tsc -b` + `npm test` +
...
### C. Engine worker wiring — **DONE + wired into the app** (`src/engine/worker/`, `src/engine/asyncEngine.ts`, `src/entry/engine.ts`)
